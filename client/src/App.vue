...
export default {
  ...
  methods: {
    ...
    logout(data) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          Swal.fire("Logged Out!", "Please come back anytime!", "success");
    ...
    editCategory(data) {
      const token = localStorage.token;
      axios
        .patch(`/task/${data.id}`, category, { headers: { token } })
        .then((data) => {
          this.reloadTask();
        })
        .catch((err) => {
    ...
    updateTask(data) {
      const token = localStorage.token;
      axios
        .put(`/task/${data.id}`, data, { headers: { token } })
        .then((data) => {
    ...
    moveCategory(payLoad) {
      ...
      const token = localStorage.token;
      axios
        .patch(`/task/${id}`, { category }, { headers: { token } })
        .then((data) => {
          this.reloadTask();
        })
        .catch((err) => {
